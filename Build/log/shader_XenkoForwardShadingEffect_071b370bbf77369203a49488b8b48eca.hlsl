...
//
#line 885 "C:\Users\brittenh\Downloads\WorldClock-master (1)\WorldClock-master\WorldClock\Bin\Windows\Debug\log\shader_XenkoForwardShadingEffect_071b370bbf77369203a49488b8b48eca.hlsl"
...
@G    Pixel => 1ff2621cb82c21a7132ecd11c4265007
//
// Generated by Microsoft (R) HLSL Shader Compiler 10.1
//
//
...
//
#line 947 "C:\Users\brittenh\Downloads\WorldClock-master (1)\WorldClock-master\WorldClock\Bin\Windows\Debug\log\shader_XenkoForwardShadingEffect_071b370bbf77369203a49488b8b48eca.hlsl"
...
#line 391
  sample_c_lz r7.y, r7.yzyy, t0.xxxx, s0, r7.w  // r7.y <- <FilterShadow_id53 return value>
...
#line 391
      sample_c_lz r2.x, r2.xyxx, t0.xxxx, s0, r2.z  // r2.x <- <FilterShadow_id53 return value>
...
#line 636
and r0.z, r0.y, l(0x0000ffff)  // r0.z <- <GetMaxLightCount_id143 return value>
...
#line 614
ushr r0.x, r0.y, l(16)  // r0.x <- <GetMaxLightCount_id155 return value>
...
#line 588
mul r0.xyz, cb0[0].xyzx, cb1[88].xyzx  // r0.x <- <ComputeEnvironmentLightContribution_id175 return value>.x; r0.y <- <ComputeEnvironmentLightContribution_id175 return value>.y; r0.z <- <ComputeEnvironmentLightContribution_id175 return value>.z
...
void CalculateAdjustedShadowSpacePixelPosition_id59(float filterRadiusInPixels, float3 pixelPositionWS, float3 meshNormalWS, float4x4 worldToShadowCascadeUV, float4x4 inverseWorldToShadowCascadeUV, out float3 adjustedPixelPositionWS, out float3 adjustedPixelPositionShadowSpace)
{
    ...
    const float4 topRightTexelWS = Project_id52(float4(ShadowMapTextureTexelSize_id88.xy * filterRadiusInPixels, 0.0, 1.0), inverseWorldToShadowCascadeUV);
...
void CalculateAdjustedShadowSpacePixelPositionPerspective_id58(float filterRadiusInPixels, float3 pixelPositionWS, float3 meshNormalWS, float4x4 worldToShadowCascadeUV, float4x4 inverseWorldToShadowCascadeUV, out float3 adjustedPixelPositionWS, out float3 adjustedPixelPositionShadowSpace)
{
    ...
    const float4 topRightTexelWS = Project_id52(float4(shadowMapCoordinate.xy + ShadowMapTextureTexelSize_id88.xy * filterRadiusInPixels, shadowMapCoordinate.z, 1.0), inverseWorldToShadowCascadeUV);
...
void ProcessLight_id82(inout PS_STREAMS streams, SpotLightDataInternal light)
{
    ...
    float attenuation = ComputeAttenuation_id81(light.PositionWS, light.AngleOffsetAndInvSquareRadius, light.DirectionWS, streams.PositionWS_id21.xyz, lightVectorNorm);
...
{
    float3 lightVectorNorm;
    float attenuation = ComputeAttenuation_id68(light, streams.PositionWS_id21.xyz, lightVectorNorm);
...
float ComputeThicknessFromCascade_id57(float3 pixelPositionWS, float3 meshNormalWS, int cascadeIndex, int lightIndex, bool isOrthographic)
{
    ...
    return FilterThickness_id54(pixelPositionWS, meshNormalWS, DepthRanges_id153[arrayIndex], WorldToShadowCascadeUV_id150[arrayIndex], InverseWorldToShadowCascadeUV_id151[arrayIndex], isOrthographic);
}
...
float4 Compute_id158(float3 direction)
{
    return 0;
}
float4 Compute_id157(float3 direction)
{
    return 0;
}
...
float3 ComputeShadow_id47(inout PS_STREAMS streams, float3 position, int lightIndex)
{
    ...
    shadowPosition += GetShadowPositionOffset_id55(OffsetScales_id155[lightIndex], streams.NdotL_id47, streams.normalWS_id18);
...
void PrepareEnvironmentLight_id168(inout PS_STREAMS streams)
{
    ...
    streams.envLightDiffuseColor_id45 = Compute_id157(sampleDirection).rgb * Intensity_id180 * ambientAccessibility * streams.matDiffuseSpecularAlphaBlend_id63.x;
    ...
    sampleDirection = float3(sampleDirection.xy, -sampleDirection.z);
    streams.envLightSpecularColor_id46 = Compute_id158(sampleDirection).rgb * Intensity_id180 * ambientAccessibility * streams.matDiffuseSpecularAlphaBlend_id63.y;
}
...
void PrepareDirectLight_id152(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.shadowColor_id81 = ComputeShadow_id75(streams, streams.PositionWS_id21.xyz, lightIndex);
...
void PrepareDirectLights_id150()
{
}
void PrepareDirectLight_id139(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.NdotL_id47 = max(dot(streams.normalWS_id18, streams.lightDirectionWS_id41), 0.0001f);
    streams.shadowColor_id81 = ComputeShadow_id64(streams, streams.PositionWS_id21.xyz, lightIndex);
    streams.lightColorNdotL_id43 = streams.lightColor_id42 * streams.shadowColor_id81 * streams.NdotL_id47 * streams.lightDirectAmbientOcclusion_id48;
    streams.lightSpecularColorNdotL_id44 = streams.lightColorNdotL_id43;
    streams.lightColorNdotL_id43 *= ComputeTextureProjection_id65(streams.PositionWS_id21.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id66, streams.normalWS_id18);
    streams.lightSpecularColorNdotL_id44 *= ComputeSpecularTextureProjection_id66(streams.PositionWS_id21.xyz, reflectionVectorWS, lightIndex);
}
int GetLightCount_id144(inout PS_STREAMS streams)
{
    return streams.lightData_id169.y & 0xFFFF;
}
int GetMaxLightCount_id143(inout PS_STREAMS streams)
{
    return streams.lightData_id169.y & 0xFFFF;
}
void PrepareDirectLights_id142(inout PS_STREAMS streams)
{
...
{
    float3 diffuseColor = streams.matDiffuseVisible_id67;
    return diffuseColor / PI_id186 * streams.lightColorNdotL_id43 * streams.matDiffuseSpecularAlphaBlend_id63.x;
}
...
void PrepareDirectLight_id115(inout PS_STREAMS streams, int lightIndex)
{
    PrepareDirectLightCore_id46(streams, lightIndex);
    streams.NdotL_id47 = max(dot(streams.normalWS_id18, streams.lightDirectionWS_id41), 0.0001f);
    streams.shadowColor_id81 = ComputeShadow_id47(streams, streams.PositionWS_id21.xyz, lightIndex);
    streams.lightColorNdotL_id43 = streams.lightColor_id42 * streams.shadowColor_id81 * streams.NdotL_id47 * streams.lightDirectAmbientOcclusion_id48;
    streams.lightSpecularColorNdotL_id44 = streams.lightColorNdotL_id43;
    streams.lightColorNdotL_id43 *= ComputeTextureProjection_id48(streams.PositionWS_id21.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id66, streams.normalWS_id18);
    streams.lightSpecularColorNdotL_id44 *= ComputeSpecularTextureProjection_id49(streams.PositionWS_id21.xyz, reflectionVectorWS, lightIndex);
}
int GetLightCount_id117()
{
    return LightCount_id83;
}
int GetMaxLightCount_id118()
{
    return TMaxLightCount_id84;
}
void PrepareDirectLights_id113()
{
}
void PrepareForLightingAndShading_id179()
{
}
void PrepareMaterialForLightingAndShading_id104(inout PS_STREAMS streams)
{
...
void ResetLightStream_id103(inout PS_STREAMS streams)
{
...
void ResetStream_id106(inout PS_STREAMS streams)
{
    ResetStream_id105();
...
void Compute_id216(inout PS_STREAMS streams)
{
    ...
    ResetLightStream_id103(streams);
    PrepareMaterialForLightingAndShading_id104(streams);
    ...
    {
        PrepareDirectLights_id113();
        const int maxLightCount = GetMaxLightCount_id118();
        int count = GetLightCount_id117();
        ...
        for (int i = 0; i < maxLightCount; i++)
        {
            ...
            PrepareDirectLight_id115(streams, i);
    ...
    {
        PrepareDirectLights_id142(streams);
        const int maxLightCount = GetMaxLightCount_id143(streams);
        int count = GetLightCount_id144(streams);
    ...
    streams.shadingColor_id71 += directLightingContribution * PI_id186 + environmentLightingContribution;
